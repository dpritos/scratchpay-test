...
export default class DentalClinic extends ClinicParent implements RepositoryContract {
  sourceUrl = config.DENTAL_CLINIC_SOURCE;
      ...
      https
        .get(this.sourceUrl, (res) => {
          res.pipe(JSONStream.parse('*')).on('data', (data: DentalClinicData) => {
            const clinic = this.transformClinic(CLINIC_TYPE.DENTAL, data);
